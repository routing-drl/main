    ...
    while len(list(graph.neighbors(node))) < max_neighbors:
        for n2 in graph.nodes:
            if n2 not in graph.neighbors(node):
                graph.add_edge(node, n2, weight=(random() * 0.9) +
                               0.1, capacity=(random() * 0.9) + 0.1)
    return graph
...
def randomize_weights(graph: nx.Graph) -> nx.Graph:
    ...
    for edge in graph.edges:
        random_edges.add_edge(edge[0], edge[1], weight=(
            random() * 0.9) + 0.1, capacity=(random() * 0.9) + 0.1)
    return random_edges
...
def compute_best_flow(graph: nx.Graph, source: int, target: int) -> float:
    ...
    while reachable:
        try:
            ...
            min_edge = min(
                flow_graph.edges, key=lambda edge: flow_graph[edge[0]][edge[1]]["capacity"])
...
def get_avg_neighbors(graph: nx.Graph):
    """
    return average latency,bw between node and all neighbors
    ...
    for node in graph.nodes():
        ...
        for neighbor in graph.neighbors(node):
            tot_lat += graph[node][neighbor]['weight']
            tot_band += graph[node][neighbor]['capacity']
            num_neighbors += 1
        tot_lat = tot_lat / num_neighbors
        tot_band = tot_band / num_neighbors
        latency_list.append(tot_lat)
        bandwidth_list.append(tot_band)

    return latency_list, bandwidth_list


def add_node_attrs(graph: nx.graph, lat_list: list, band_list: list):
    for node, lat, band in zip(graph.nodes(), lat_list, band_list):
        graph.nodes[node]['weight'] = lat
        graph.nodes[node]['capacity'] = band
